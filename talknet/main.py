...
from talknet.model.faceDetector.s3fd import *
from talknet.talkNet import TalkNet

warnings.filterwarnings("ignore")

pretrained_model_path = "/tmp/asd/models/pretrain_TalkSet.model"
save_path = "save/"
...
def predict_faces(DET, start_frame=0):
    ...
    for fidx, fname in enumerate(flist):
        ...
        dets.append([])
        for bbox in bboxes:
            dets[-1].append({'frame': fidx + start_frame, 'bbox': (bbox[:-1]).tolist(),
    ...
    while True:
        ...
        if track == []:
            ...
            if max(numpy.mean(bboxesI[:, 2] - bboxesI[:, 0]),
                   numpy.mean(bboxesI[:, 3] - bboxesI[:, 1])) > min_face_size:
...
def talknet_work(
    ...
    command = (
            "ffprobe -v error -select_streams v:0 -count_packets -show_entries stream=nb_read_packets -of csv=p=0 '%s'" % video_path)
    ...
    # get video length
    command = (
            "ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 '%s'" % video_path)
    ...
    # get fps
    command = (
            "ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate -of default=noprint_wrappers=1:nokey=1 '%s'" % video_path)
            ...
            try:
                ...
                try:
                    ...
                    command = ("ffmpeg -y -i '%s' -vf fps=25 -ss %s -t %s -threads %d -f image2 %s -loglevel panic" % \
                               (
                                   video_path, start, duration, data_loader_thread,
                                   os.path.join(pyframesPath, '%06d.jpg')))
                    subprocess.call(command, shell=True, stdout=None)
                    # read the frames from the extracted video frames
                    frames = [cv2.imread(os.path.join(pyframesPath, '%06d.jpg' % i)) for i in
                              range(int(duration * fps))]
    ...
    for scene_data in scene:
        new_scenes.append((FrameTimecode(timecode=int(scene_data[0].frame_num * (25 / fps)), fps=25),
                           FrameTimecode(timecode=int(scene_data[1].frame_num * (25 / fps)), fps=25)))
    ...
    if face_boxes:
        ...
        for face_box in face_boxes:
            ...
            for i in range(1, len(frame_data), 5):
                bbox = [float(frame_data[i]), float(frame_data[i + 1]), float(frame_data[i + 2]),
    ...
    for shot in new_scenes:
        if shot[1].frame_num - shot[0].frame_num >= min_track:  # Discard the shot frames less than minTrack frames
            allTracks.extend(track_shot(faces[max(0, shot[0].frame_num - first_frame):shot[
                                                                                          1].frame_num - first_frame]))  # 'frames' to present this tracks' timestep, 'bbox' presents the location of the faces
    ...
    for tidx, track in enumerate(vidTracks):
        ...
        for fidx, frame in enumerate(track['track']['frame'].tolist()):
            ...
            faces[frame]['faces'].append(
                {'track_id': tidx, 'raw_score': float(s), 'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2,
                 'speaking': bool(s >= 0)})
...
class TalkNetASD:
    ...
    def predict(
        ...
        """
        :param video_path: a video to process
        ...
        if return_visualization:
            result_out, video_path = talknet_work(self.s, self.DET, video_path, start_seconds=start_time, end_seconds=end_time,
                                          return_visualization=return_visualization, face_boxes=face_boxes,
                                          in_memory_threshold=in_memory_threshold)
            return video_path
        else:
            result_out = talknet_work(self.s, self.DET, video_path, start_seconds=start_time, end_seconds=end_time,
                              return_visualization=return_visualization, face_boxes=face_boxes,
                              in_memory_threshold=in_memory_threshold)
            return transform_out(result_out)


if __name__ == "__main__":
    TEST_URL = "/home/kapsi/Downloads/video.mp4"
    model = TalkNetASD()
    # change "url" to "path" if you want to test with a local file
    out = model.predict(video_path=TEST_URL, return_visualization=True)
    print(list(out))
